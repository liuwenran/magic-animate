...
def main(args):
    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    if config.savename is None:
        ...
        savedir = f"samples/{config.savename}--{time_str}"
    ...
    video_names = [ path for path in os.listdir(config.source_video_path[0])]
    video_names.sort()
    test_videos = [ os.path.join(config.densepose_video_path[0], video_name) for video_name in video_names]
    source_image_paths = [ os.path.join(config.source_video_path[0], video_name) for video_name in video_names]
    ...
    for idx, (source_image, test_video, random_seed, size, step) in tqdm(
        enumerate(zip(source_image_paths, test_videos, random_seeds, sizes, steps)), 
        ...
        if test_video.endswith('.mp4'):
            try:
                ...
                continue
            evaluate_inds = list(range(0, len(control)))
            if len(evaluate_inds) > 72:
                evaluate_inds = evaluate_inds[::3]
            evaluate_inds = evaluate_inds[:24]
            control = [np.array(Image.fromarray(control[c]).resize((size, size))) for c in evaluate_inds]
        ...
        if args.rank == 0:
            ...
            save_videos_grid(samples_per_video[-1:], f"{savedir}/videos_{config.savename}_{video_name}_0.mp4")
            # save_videos_grid(samples_per_video, f"{savedir}/videos_{config.savename}_{video_name}_0_grid.mp4")
